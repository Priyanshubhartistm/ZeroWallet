...
function StoryMode({ tx }: { tx: TransactionResult }) {
  ...
  const steps = [
    ...
    {
      title: "ZeroWallet took ownership",
      description: "Instructions were passed to ZeroWallet for secure execution. It wrapped your intent into a valid Solana transaction."
    },
    {
    ...
    <section className="mb-10">
      ...
      <div className="space-y-3">
        {tx.instructions.map((ix, index) => {
          ...
          let explanation = 'Executed a ZeroWallet chunk instruction which processed session logic and triggered an inner SOL transfer.';
          let affected = "Smart Wallet, Wallet State, Chunk account, and recipient account (balances changed)."
          let reason = "To execute a chunked operation inside the ZeroWallet smart wallet, including refunding or transferring SOL as part of session execution.";
  ...
  return (
    <section className="mb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* ZeroWallet */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">ZeroWallet</h3>
          <ul className="space-y-2 text-sm">
            <li className="flex items-center gap-2 text-muted-foreground">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" />
              Built transaction
